...
public sealed partial class HistoryPage : Page
{
    ...
    private async void SaveFromHistory(object sender, RoutedEventArgs e)
    {
        ...
        if (historyFolder != null)
        {
            ...
            if (files.Count > 0)
            {
                ...
                if (Directory.Exists(imageFolder.Path)) await Launcher.LaunchFolderAsync(imageFolder);

                if (Directory.Exists(textFolder.Path)) await Launcher.LaunchFolderAsync(textFolder);
            }
        }
    }
        ...
        if (historyFolder != null)
        {
            ...
            if (files.Count > 0)
            {
                ...
                var deleteHistoryDialog = new ContentDialog
                {
                    ...
                    Content = "Are you sure you want to delete " + selectedItems.Count() +
                ...
                if (result == ContentDialogResult.Primary)
                {
                    ...
                    while (HistoryList.SelectedItems.Count > 0) HistoryList.Items.Remove(HistoryList.SelectedItem);
    ...
    private async void LoadHistory(object sender, RoutedEventArgs e)
    {
        ...
        {
            var enableHistoryDialog = new ContentDialog
            {
            ...
            enableHistoryDialog.XamlRoot = XamlRoot;

            var result = await enableHistoryDialog.ShowAsync();
            //if the user clicks the delete button, delete the history
            if (result == ContentDialogResult.Primary)
                ...
                if (files.Count > 0)
                {
                    ...
                    Array.Sort(imageFiles,
                        new Comparison<string>((x, y) =>
                            DateTime.Compare(File.GetCreationTime(y), File.GetCreationTime(x))));
                    var imageCount = imageFiles.Length;

                    var i = 0;
                    while (i < imageCount)
                    {
                        var imageFile = await StorageFile.GetFileFromPathAsync(imageFiles[i]);
                        if (imageFile == null)
                        {
                            i++;
                            continue;
                        ...
                        var imageFileName = imageFile.DisplayName;

                        var imageFileNameNoExtension = Path.GetFileNameWithoutExtension(imageFileName);
                        ...
                        var textFile =
                            await StorageFile.GetFileFromPathAsync(historyFolder.Path + "\\" +
                                                                   imageFileNameNoExtension + ".txt");
                        var textFileName = textFile.DisplayName;
                        var textFileNameNoExtension = Path.GetFileNameWithoutExtension(textFileName);
                        ...
                        if (imageFileNameNoExtension == textFileNameNoExtension)
                        {
                            ...
                            image.RightTapped += (s, e) =>
                            {
                                ...
                                CopyImageMenuButton.Click += async (s, e) =>
                                {
                                    ...
                                    Clipboard.SetContentWithOptions(dataPackage,
                                ...
                                OpenImageMenuButton.Click += async (s, e) =>
                                {
                                    ...
                                    await Launcher.LaunchFileAsync(
                                        await StorageFile.GetFileFromPathAsync(imageFile.Path), options);
                                };
                            };
